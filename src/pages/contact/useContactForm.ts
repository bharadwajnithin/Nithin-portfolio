...
const useContactForm = () => {
   ...
   const handleSubmit = useCallback(
      async (e: React.FormEvent<HTMLFormElement>) => {
         ...
         // Email service credentials are intentionally not stored in a public
         // portfolio. Open the visitor's mail client when no EmailJS service is
         // configured so the contact form remains useful.
         if (!emailConfig.service_id || !emailConfig.template_id || !emailConfig.public_key) {
            ...
            globalThis.location.href = `mailto:${getEmail()}?subject=${subject}&body=${body}`;
            return;
         }
